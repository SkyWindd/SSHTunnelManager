...
// ============================================================
//  LinuxApp — CLI menu shell cho Linux/Ubuntu
//  Tương đương WindowsApp.cs nhưng dùng:
//    - LinuxTunnelMonitor thay TunnelMonitor
//    - LinuxConnectionHandler thay ConnectionHandler
//    - SshWrapper thay PlinkWrapper
// ============================================================

public static class LinuxApp
{
    private static AppConfig _cfg = null!;
    private static LinuxTunnelMonitor _monitor = null!;
    ...
    public static void Run(string[] args)
    {
        ...
        if (!SshWrapper.ValidateSshPath())
        {
            Console.ForegroundColor = ConsoleColor.Red;
            Console.WriteLine("\n  ✘ Không tìm thấy 'ssh' trên hệ thống.");
            Console.WriteLine("    Cài OpenSSH client: sudo apt install openssh-client");
            Console.ResetColor();
        }
        ...
        // --encrypt-key: tool để mã hóa file .pem (người quản lý dùng 1 lần)
        ...
        if (!SshWrapper.ValidatePemKey(_cfg.Vps))
        {
            Console.ForegroundColor = ConsoleColor.Yellow;
            Console.WriteLine($"\n  ⚠  Không tìm thấy file key .pem.");
            Console.WriteLine($"     Đặt file 'default_vps.pem' cạnh SshTunnelManager.");
            Console.WriteLine($"     Nếu chỉ có file .ppk, convert bằng lệnh:");
            Console.WriteLine($"     puttygen default_vps.ppk -O private-openssh -o default_vps.pem");
            Console.ResetColor();
        }
        ...
        _monitor = new LinuxTunnelMonitor(_cfg);
    ...
    private static void RunMenu()
    {
        while (true)
        {
            ...
            switch (choice)
            {
                ...
                case "4": LinuxConnectionHandler.PrintTunnelUsageGuide(_cfg);    break;
    ...
    private static void OpenClientApp()
    {
        ...
        Console.WriteLine("\n  Open client app:");
        Console.WriteLine("  [1] SSH  (mở terminal với lệnh ssh)");
        Console.WriteLine("  [2] RDP  (hướng dẫn Remmina / xfreerdp)");
        Console.Write("Choice: ");
        switch (Console.ReadLine()?.Trim())
        {
            case "1": LinuxConnectionHandler.PrintSshCommand(_cfg); break;
            case "2": LinuxConnectionHandler.PrintRdpGuide(_cfg);   break;
    ...
    private static void ManageCustomTunnels()
    {
        ...
        switch (Console.ReadLine()?.Trim())
        {
            case "1": LinuxConnectionHandler.AddCustomTunnel(_cfg);  break;
            case "2": LinuxConnectionHandler.RemoveTunnel(_cfg);     break;
            default:  Console.WriteLine("Cancelled.");               break;
        }
    }
    ...
    /// Người quản lý dùng 1 lần, sau đó phân phối file .pem.enc.
    /// </summary>
    private static void RunEncryptKeyTool()
    {
        ...
        Console.WriteLine("  Tool này mã hóa 'default_vps.pem' → 'default_vps.ppk.enc'");
        Console.WriteLine("  File .enc có thể upload GitHub an toàn.");
        Console.WriteLine("  File .pem gốc SAU KHI MÃ HÓA nên xóa khỏi thư mục publish.\n");

        if (KeyManager.DetectMode() == KeyMode.Missing)
        {
            Console.ForegroundColor = ConsoleColor.Red;
            Console.WriteLine("  ✘ Không tìm thấy 'default_vps.pem'.");
            Console.WriteLine("     Đặt file .pem vào cùng thư mục với SshTunnelManager rồi chạy lại.");
            Console.ResetColor();
            Console.ReadLine();
            return;
        }
        ...
        {
            Console.ForegroundColor = ConsoleColor.Yellow;
            Console.WriteLine("  ℹ  File đã tồn tại.");
        ...
        try
        {
            ...
            Console.ForegroundColor = ConsoleColor.Green;
            Console.WriteLine("\n  ✔  Mã hóa thành công!");
            Console.WriteLine("  ✔  File '.enc' đã sẵn sàng phân phối.");
            Console.ResetColor();
            Console.ForegroundColor = ConsoleColor.Yellow;
            Console.WriteLine("\n  ⚠  Tiếp theo:");
            Console.WriteLine("     1. XÓA file 'default_vps.pem' khỏi thư mục publish/");
            Console.WriteLine("     2. Giữ lại file '.enc'");
            Console.WriteLine("     3. Thông báo Group Password cho thành viên qua kênh riêng");
  ...
  ╚═══════════════════════════════════════════════════════╝");
    ...
    private static void PrintMenu()
    {
        ...
        if (_cfg?.Role == MachineRole.MachineA)
            Console.WriteLine("  [5] Open SSH / RDP");
