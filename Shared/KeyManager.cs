...
public static class KeyManager
{
    ...
    /// <summary>Xóa file .ppk tạm khỏi đĩa, thử lại nếu bị lock.</summary>
    public static void DeleteTempKey(string path)
    {
        if (string.IsNullOrEmpty(path) || !File.Exists(path)) return;

        // Thử xóa tối đa 5 lần, mỗi lần chờ 300ms để plink.exe nhả lock
        ...
        // Nếu vẫn không xóa được — đăng ký xóa khi process thoát
        try
        {
            if (OperatingSystem.IsWindows())
            {
                // Windows: dùng MoveFileEx để đăng ký xóa lúc reboot
                MoveFileExWindows(path, null, 4);
                Logger.Info("File key tạm sẽ được xóa khi khởi động lại Windows.");
            }
            else
            {
                // Linux/macOS: đăng ký xóa khi process exit
                AppDomain.CurrentDomain.ProcessExit += (_, _) =>
                {
                    try { if (File.Exists(path)) File.Delete(path); } catch { /* ignore */ }
                };
                Logger.Info("File key tạm sẽ được xóa khi thoát app.");
            }
        }
        catch
        {
            Logger.Warn($"File key tạm chưa xóa được: {path} — hệ thống sẽ tự dọn sau.");
        }
    }

    [System.Runtime.InteropServices.DllImport("kernel32.dll", SetLastError = true, CharSet = System.Runtime.InteropServices.CharSet.Unicode)]
    [System.Runtime.Versioning.SupportedOSPlatform("windows")]
    private static extern bool MoveFileExWindows(string lpExistingFileName, string? lpNewFileName, int dwFlags);
